...
You can download the datafile from the module VLE page under week 1>Lab Materials or at this link [Github Link](https://github.com/vrNeuro/R_Teaching_Materials/blob/main/data/WHR2023.csv). Save this file in the same folder as your script and project file. You should see it appear in the Files tab in the lower right.

::: {.callout-tip}
## Uploading for RStudio Server
...
```{r}
#| echo: false
mydata <- read.csv("data/WHR2023.csv")
```
...
```
The sections of these workbooks that look like that above will be executed when the file is created. This enables the analysis results to be incorporated into the worksheets.
...
![Figure 4: Our script with the loading included](images/FirstScriptLoad.png)
To run a line/s of code, you simply highlight the relevant lines, and then click the run button in the top right of the upper left pane. The keyboard shortcut is hold CTRL (Command on MACs) and press ENTER. 

This will send the code to the R console in the lower left. You should see the syntax replicated in the R console window. Go ahead and run this code. 
...
You can also see that in the console the command `View(mydata)` was executed to do this. Often when you click buttons in RStudio, it automatically executes code for you which can be a useful way to learn new commands.

![Figure 5: The mydata dataframe in the variable viewer](images/mydataInVarView.png)
...
![Figure 6: The mydata dataframe expanded](images/ExpandedVariable.png)
...
**What get's displayed in the console?**
<textarea rows="2" cols="60" placeholder="Type your answer here..."></textarea>
...
<textarea rows="3" cols="60" placeholder="Type your answer here..."></textarea>
...
::: {.callout-note collapse="true" title="Show answer"}
The happiness scores are reasonably normally distributed, although we would need to check some statistics (more on this in coming weeks). The issue that is perhaps more problematic is that there appear to be two outliers at the bottom end of the distribution ie hovering around 2. These two values have moderate separation from the next lowest values, which are around 3.  
:::
...
```{r}
hist(mydata$Ladder.score, main = "Histogram of Happiness Scores", xlab = "Happiness Scores", col = "blue")
```
...
```{r}
boxplot(mydata$Ladder.score, main = "Boxplot of Happiness Scores", ylab = "Happiness Scores")
```
...
If you copied the commands used in this document to your own R script, you should now be able to run this simple analysis yourself. You will be able to find an example of this complete R script on the VLE after Wednesday.
